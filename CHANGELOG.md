...
| `jira__daily_sprint_issue_history` | Removed column | Included `updated_at` column | Column removed | **Breaking:** The `updated_at` field has been removed due to logic redesign. Previously tracked timestamp when sprint assignment was updated. New logic uses date-range based membership instead of tracking individual update events. Users needing sprint assignment change tracking can join to `int_jira__issue_multiselect_history`. [PR #169](https://github.com/fivetran/dbt_jira/pull/169) |
