...
| [jira__sprint_enhanced](https://fivetran.github.io/dbt_jira/#!/model/model.jira.jira__sprint_enhanced) | One row per sprint per team with metrics on issues created, resolved, and carried over. If teams are disabled, granularity is one row per sprint. <br><br>**Example Analytics Questions:**<br><ul><li>Which sprints had the highest velocity and what made them successful?</li><li>How many issues were planned vs. completed across recent sprints?</li><li>What percentage of issues are typically carried over from sprint to sprint?</li></ul> |
...
The `jira__daily_issue_field_history` and `jira__timestamp_issue_field_history` models generate historical data for the columns specified by the `issue_field_history_columns` variable. By default, the columns tracked in `jira__daily_issue_field_history` and `jira__timestamp_issue_field_history` are `status`, `status_id`, and `sprint`, but all fields found in the Jira `FIELD` table's `field_name` column can be included in these models. The most recent value of any tracked column is also captured in `jira__issue_enhanced`.
