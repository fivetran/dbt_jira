...
calculate_medians as (
    ...
    select 
        ...
        round(cast({{ fivetran_utils.percentile(percentile_field='case when resolved_at is not null then open_duration_seconds end', 
                    partition_field='current_sprint_id', percent='0.5') }} as {{ dbt.type_numeric() }} ), 0) as median_close_time_seconds,
        round(cast({{ fivetran_utils.percentile(percentile_field='case when resolved_at is null then open_duration_seconds end', 
                    partition_field='current_sprint_id', percent='0.5') }} as {{ dbt.type_numeric() }} ), 0) as median_age_currently_open_seconds,
        round(cast({{ fivetran_utils.percentile(percentile_field='case when resolved_at is not null then any_assignment_duration_seconds end', 
                    partition_field='current_sprint_id', percent='0.5') }} as {{ dbt.type_numeric() }} ), 0) as median_assigned_close_time_seconds,
        round(cast({{ fivetran_utils.percentile(percentile_field='case when resolved_at is null then any_assignment_duration_seconds end', 
                    partition_field='current_sprint_id', percent='0.5') }} as {{ dbt.type_numeric() }} ), 0) as median_age_currently_open_assigned_seconds
    from issue
    {% if target.type == 'postgres' %} group by current_sprint_id {% endif %}
),
...
join_metrics as (
    ...
    left join median_metrics 
        on calculate_avg_metrics.current_sprint_id = median_metrics.current_sprint_id
)
