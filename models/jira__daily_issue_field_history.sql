    ...
    select
        ...
        {% if is_incremental() %}    
            {% for col in pivot_data_columns %}
                ...
                {% elif col.name|lower not in ['issue_day_id', 'issue_id', 'valid_starting_on', 'updated_at_week', 'components'] %} 
...
set_values as (
    select
        ...
        {% set exception_cols = ['issue_id', 'issue_day_id', 'valid_starting_on', 'updated_at_week', 'status', 'status_id', 'components', 'issue_type'] %}
...
fill_values as (
    ...
    select  
        ...
        {% for col in pivot_data_columns %}
            ...
            {% elif col.name|lower not in ['issue_id', 'issue_day_id', 'valid_starting_on', 'updated_at_week', 'status', 'status_id', 'components'] %}
...
fix_null_values as (
    ...
    select  
        ...
        {% for col in pivot_data_columns %}
            ...
            {% elif col.name|lower not in ['issue_id','issue_day_id','valid_starting_on', 'updated_at_week', 'status', 'components'] %}
